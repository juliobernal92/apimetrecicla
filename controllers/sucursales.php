...
switch ($_SERVER['REQUEST_METHOD']) {
    case 'GET':
        if (isset($_COOKIE['jwt'])) {
            ...
            try {
                ...
                if (isset($_GET['id'])) {
                    $id_sucursal = $_GET['id'];
                    $resultado = $sucursalModel->get_sucursal_by_id($id_sucursal);
                    echo get_resultado($resultado);
                } else {
                    $resultado = $sucursalModel->get_sucursales();
                    echo get_resultado($resultado);
                }
            } catch (Exception $e) {
    ...
    case 'POST':
        if (isset($_COOKIE['jwt'])) {
            ...
            try {
                $userData = decode($token);
                $resultado = $sucursalModel->insertar_sucursal($body);
                echo get_resultado($resultado);
            } catch (Exception $e) {
                http_response_code(401);
                echo json_encode(["message" => "Acceso Denegado.", "error" => $e->getMessage()]);
    ...
    case 'PUT':
        if (isset($_COOKIE['jwt'])) {
            ...
            try {
                $userData = decode($token);
                $resultado = $sucursalModel->actualizar_sucursal($body);
                echo get_resultado($resultado);
            } catch (Exception $e) {
                http_response_code(401);
                echo json_encode(["message" => "Acceso Denegado.", "error" => $e->getMessage()]);
    ...
    case 'DELETE':
        if (isset($_COOKIE['jwt'])) {
            ...
            try {
                ...
                if (isset($_GET['id'])) {
                    $id_sucursal = $_GET['id'];
                    $resultado = $sucursalModel->eliminar_sucursal($id_sucursal);
                    echo get_resultado($resultado);
                } else {
