...
switch ($_SERVER['REQUEST_METHOD']) {
    case 'GET':
        if (isset($_COOKIE['jwt'])) {
            ...
            try {
                ...
                if (isset($_GET['id'])) {
                    $id_compra = $_GET['id'];
                    $resultado = $comprasModel->get_compra_by_id($id_compra);
                    echo get_resultado($resultado);
                } else {
                    $resultado = $comprasModel->get_compras();
                    echo get_resultado($resultado);
                }
            } catch (Exception $e) {
    ...
    case 'POST':
        if (isset($_COOKIE['jwt'])) {
            ...
            try {
                $userData = decode($token);
                $resultado = $comprasModel->insertar_compra($body);

                if ($resultado['success']) {
                    echo get_resultado(['id_compra' => $resultado['id_compra']]);
                } else {
                    http_response_code(500);
                    echo json_encode([
                        'codigo' => 500,
                        'descripcion' => 'Error al insertar la compra.',
    ...
    case 'PUT':
        if (isset($_COOKIE['jwt'])) {
            ...
            try {
                $userData = decode($token);
                $resultado = $comprasModel->actualizar_compra_total($body);
                echo get_resultado($resultado);
            } catch (Exception $e) {
                http_response_code(401);
                echo json_encode(["message" => "Acceso Denegado.", "error" => $e->getMessage()]);
    ...
    case 'DELETE':
        if (isset($_COOKIE['jwt'])) {
            ...
            try {
                ...
                if (isset($_GET['id'])) {
                    $id_compra = $_GET['id'];
                    $resultado = $comprasModel->eliminar_compra($id_compra);
                    echo get_resultado($resultado);
                } else {
